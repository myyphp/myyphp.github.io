...
项目中有个这样的需求：为客户生成报表时，是根据每种类型生成了一个报表，最后通过 `PHPZip` 类实现打包。
...
```php
$PHPZip = new \PHPZip();
$PHPZip->ZipFile($pack_files_arr, $zip_name, 2, $zip_save_path);
```


# 解决方法

此时，可以使用php的zip扩展类: `ZipArchive` 实现压缩，可以在添加压缩文件时，指定重命名的文件名：

```php
$zip = new \ZipArchive();
if ($zip->open($zip_save_path . $zip_name, \ZipArchive::CREATE) === true) {
    foreach ($zip_arr as $v) {
        //$v为待打包文件的路径
        //$refilename 对文件进行重命名
        $zip->addFile($v, $refilename);
    }
    $zip->close();
}
```
...
class PHPZip
{
    ...
    function ZipDir($dir, $zipfilename, $Todo, $save_paht)
    {
        ...
        Switch ($Todo) {
            Case "1":
                $this->DownLoad($save_paht, $zipfilename, $out);
                Break;
            Case "2":
                $this->SaveFile($save_paht, $zipfilename, $out);
                Break;
        }
    }
    ...
    function ZipFile($file, $zipfilename, $Todo, $save_paht)
    {
        IF (!@function_exists('gzcompress')) {
            Return 0;
        }
